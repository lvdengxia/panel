<template>
  <div class="le-matter">
    ...
    <button class="he-btn he-primary-btn le-new-add" @click="router()">
      新建优惠券
    </button>
    <el-form
      class="le-card"
      ...
      >
        ...
        <el-table-column label="优惠内容" prop="content" min-width="200px">
          <template slot-scope="scope">
            <template v-if="Number(scope.row.min_price) === 0">
              无门槛使用，减{{ scope.row.sub_price }}
            </template>
            <template v-else>
              满{{ scope.row.min_price }}减{{ scope.row.sub_price }}
            </template>
          </template>
        </el-table-column>
        <el-table-column label="已领取/剩余" width="200px">
          <template slot-scope="scope">
            {{ scope.row.obtain_num }}/{{ scope.row.over_num }}
          </template>
        </el-table-column>
        <el-table-column
          label="已使用"
        ...
        <el-table-column label="用券时间" width="200px">
          <template slot-scope="scope">
            <template v-if="scope.row.expire_type === 1"
              >领取后{{ scope.row.expire_day }}天内</template
            >
        ...
        <el-table-column label="状态" width="150px">
          <template slot-scope="scope">
            <div
              class="le-table-status le-status-success"
              v-if="scope.row.status === 1"
            >
              上架中
            </div>
            <div
              class="le-table-status le-status-gray"
              v-else-if="scope.row.status === 0"
            >
              下架中
            </div>
            <div
              class="le-table-status le-status-caveat"
          ...
          <template slot-scope="scope">
            <div class="le-table-edit flex align-center">
              ...
              <template v-if="scope.row.status === 1">
                <span class="le-button-text" @click="downCoupon(scope.row)"
                  >下架</span
                >
                <span class="le-line-text"></span>
                <span
                ...
                >
                <span class="le-line-text"></span>
                <span class="le-button-text" @click="replace(scope.row)"
                  >发放</span
                >
              </template>
              <template v-if="scope.row.status === 0">
                <span class="le-button-text" @click="putCoupon(scope.row)"
                  >上架</span
                >
                <span class="le-line-text"></span>
              </template>
              <template v-if="scope.row.status === 2 || scope.row.status === 0">
                <template v-if="scope.row.status === 2">
                  <span class="le-button-text" @click="router(scope.row.id, 1)"
                    >查看</span
                  >
                  <span class="le-line-text"></span>
                </template>
                <span
                  class="le-button-text"
                  @click="delCoupon(scope.row.id, scope.$index)"
                  >删除</span
                >
              </template>
            </div>
...
export default {
  ...
  async mounted() {
    ...
    this.getList(
      this.$route.query.current ? parseInt(this.$route.query.current) : 1
    );
  },
  methods: {
    ...
    downCoupon: function (row) {
      let _this = this;
      this.$confirm(
        "优惠券下架后，用户不可再领取，但已领取过的用户，仍可继续使用优惠券，是否确认下架？",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
