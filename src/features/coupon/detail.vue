        ...
        <div class="le-left-top">
          <el-scrollbar class="le-left-top_content">
            <div class="le-coupon-detail">
              <div
                class="le-coupon-share align-center flex flex-direction"
                v-if="form.enable_share"
              >
              ...
              <div class="le-coupon-detail-top">
                ...
                <div
                  class="le-coupon-heade-body flex flex-direction align-center"
                >
              ...
              <div class="le-coupon-detail-bottom">
                <div class="le-coupon-bottom-body">
                  ...
                  <div class="le-coupon-text">
                    <template v-if="form.time.expire_type === 1">
                      领取后{{ form.time.expire_day }}天内
                    </template>
                    <template v-else>
                      <template
                        v-if="form.time.begin_time && form.time.end_time"
                      >
                        {{
                      ...
                      <template v-else>
                        XXXX.XX.XX XX:XX:XX - XXXX.XX.XX XX:XX:XX</template
                      >
                    </template>
                  </div>
        ...
        <div class="le-card">
          ...
          <div class="le-card-body">
            ...
            <el-form-item prop="total_num">
              ...
              <div class="le-item-form-prompt">
                编辑发放总量时只能增加不能减少，请谨慎设置
              </div>
            </el-form-item>
            <el-form-item prop="time" class="le-form-item-time">
              ...
              <div class="flex">
                ...
                <div>
                  ...
                  <div class="le-item-form-prompt">
                    若当前时间超过优惠券的固定用券时间，则优惠券状态将变成已结束。
                  </div>
                </div>
              </div>
              <div class="flex">
            ...
            <el-form-item prop="appoint_data">
              ...
              <div class="le-des-goods" v-if="form.appoint_type === 2">
                <template v-if="!examine">
                  <el-button @click="getGoods" class="le-select-btn"
                    >选择商品</el-button
                  >
                  ...
                  <el-table-column label="商品">
                    <template slot-scope="scope">
                      <div class="le-table-goods flex align-center">
                        <img
                          :src="scope.row.slideshow && scope.row.slideshow[0]"
                          alt=""
                        />
                  ...
                  <el-table-column label="操作" width="80" v-if="!examine">
                    <template slot-scope="scope">
                      <span
                        ...
                        @click="form.appoint_data.splice(scope.$index, 1)"
                        >删除</span
                      >
                    </template>
                  </el-table-column>
              ...
              <div>
                ...
                <div class="le-des-group" v-if="form.appoint_type === 3">
                  ...
                  <div class="le-form-group">
                    <span
                      class="le-group-item"
                      v-for="(item, index) in group.available.result"
                      :key="index"
                      >{{ item.value }}
                      <he-icon
              ...
              <div>
                ...
                <div class="le-des-group" v-if="form.appoint_type === 5">
                  ...
                  <div class="le-form-group">
                    <span
                      class="le-group-item"
                      v-for="(item, index) in group.unavailable.result"
                      :key="index"
                      >{{ item.value }}
                      <he-icon
                        class="le-group-item-delete"
        ...
        <div class="le-card">
          ...
          <div class="le-card-body">
            <el-form-item prop="give_limit">
              ...
              <div class="le-item-form-prompt">
                仅对用户主动领取有效，自动发放不生效，不填写则表明不限制
              </div>
            </el-form-item>
            <el-form-item>
              <span slot="label" class="le-form-item__label">分享设置</span>
              <el-checkbox
                v-model="form.enable_share"
                :true-label="1"
                :false-label="0"
                :disabled="examine"
              >
            ...
            <el-form-item prop="register_limit.value">
              ...
              <el-checkbox
                v-model="form.register_limit.type"
                :true-label="1"
                :false-label="0"
              >
        ...
        <div class="le-card">
          ...
          <div class="le-card-body">
            ...
            <el-form-item>
              ...
              <div class="le-item-form-prompt">
                勾选，则因购买商品而发放的优惠券，在商品退款成功后，优惠券失效
              </div>
            </el-form-item>
            <el-form-item>
...
export default {
  ...
  data() {
    const checkTotal = (rule, value, callback) => {
      if (this.$route.query.id) {
        if (value < this.total_num) {
          callback(
            new Error(
              "发放数量只能增加不能减少，请填写" + this.total_num + "以上的数字"
            )
          );
        }
      }
    ...
    return {
      ...
      rules: {
        ...
        time: [
          { required: true, validator: checkTime, trigger: ["blur", "change"] },
        ],
        min: [
        ...
        appoint_data: [
          { required: true, validator: checkAppointData, trigger: ["blur"] },
        ],
        give_limit: [
          { validator: checkGiveLimit, trigger: ["blur", "change"] },
        ],
        expire_remind: [
          { validator: checkExpireRemind, trigger: ["blur", "change"] },
        ],
        "register_limit.value": [
          { validator: checkRegisterLimit, trigger: ["blur", "change"] },
        ],
      },
    ...
    submit: function () {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          for (let key in form) {
            if ((!form[key] || form[key] == "") && form[key] !== 0)
              delete form[key];
          }
          if (form.expire_type === 2) {
            ...
            form.begin_time = parseInt(
              form.begin_time.substring(0, form.begin_time.length - 3)
            );
          }
          if (form.min.type === 0) {
            form.min_price = 0;
  ...
  watch: {
    ...
    "form.give_limit": {
      handler(val) {
        if (this.loading) return;
        this.receiveContentStr = `领取规则：${
          val ? "每人限领" + val + "张" : "无限制领取"
        }`;
        this.form.content = this.userContentStr + this.receiveContentStr;
      },
    },
