<template>
    <div class="le-matter le-chooseGroup" v-loading="loading">
        <div class="le-cat__head">
            <el-autocomplete placeholder="输入分类名称搜索"
                             @select="getSelect"
                             clearable
                             @clear="getClear"
                             ref="select"
                             :fetch-suggestions="suggestions" v-model="state">
        ...
        <div class="le-cat__body flex">
            <div class="le-cat-item__first flex-sub">
                <div class="le-cat-item__search">
                    <el-autocomplete placeholder="输入一级分类名称"
                ...
                <el-scrollbar class="le-cat-item-bar">
                    <template v-for="(item, index) in leftList">
                        <div class="le-cat-item flex" :key="index"
            ...
            <div class="le-cat-item__second flex-sub">
                <div class="le-cat-item__search">
                    <el-autocomplete placeholder="输入二级分类名称"
                ...
                <el-scrollbar class="le-cat-item-bar">
                    <template v-for="(item, index) in rightList">
                        <div class="le-cat-item flex" :key="index"
                             v-show="item.show"
                             @click="setChecked(item, 'rightList')">
                            <div class="le-cat-item--check">
                                <he-icon type="le-icon-tick" size="8px" v-if="item.checked"></he-icon>
                            </div>
    ...
    methods: {
        getList: function () {
            ...
            return new Promise(function (resolve, reject) {
                _this.$heshop.group('get', {include: 'goods'}).then(function (res) {
                    let list = [];
        ...
        setList: function () {
            ...
            this.rightList = this.list[this.leftIndex].list && this.list[this.leftIndex].type === 3 ? this.list[this.leftIndex].list : [];
        },
        ...
        querySearch: function (queryString, cb, list) {
            let results = queryString ? this[list].filter(function (restaurant) {
                return (restaurant.value.toLowerCase().indexOf(queryString.toLowerCase()) >= 0);
            }) : this[list];
            cb(results);
        },
        suggestions: function (str, cb) {
            this.querySearch(str, cb, 'searchList');
        },
        suggestionsLeft: function (str, cb) {
            this.querySearch(str, cb, 'leftList');
            console.log(this.leftList)
        },
        suggestionsRight: function (str, cb) {
            this.querySearch(str, cb, 'rightList');
        },
