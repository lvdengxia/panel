<template>
  <div class="le-goods">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <template v-if="scope.row.stocks > 0">
            <el-tag size="medium" type="success" v-if="scope.row.is_sale == 1"
              >销售中</el-tag
            >
            <el-tag size="medium" type="info" v-else-if="scope.row.is_sale == 0"
              >下架中</el-tag
            >
          </template>
      ...
      >
        <template slot-scope="scope">
          <div class="le_goods__main-goodstitle">
            访问量：{{ scope.row.visits }}
          </div>
          <div class="le_goods__main-goodstitle">
            访客数：{{ scope.row.visitors }}
          </div>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <div class="le-button-editor el-row--flex he-btn flex align-center">
            ...
            <template v-else>
              ...
              <popconfirm
                @confirm="
                  recycleDelete({ id: scope.row.id, index: scope.$index })
                "
                title="商品将被删除且无法撤销，确定删除？"
                width="392px"
              >
    ...
    <div
      class="le-goods__footer el-row--flex is-align-middle is-justify-space-between"
    >
      <div>
        <el-checkbox
          :indeterminate="indeterminate"
...
export default {
  ...
  methods: {
    ...
    selectionChange: function (rows) {
      ...
      this.indeterminate =
        this.checkList.length > 0 && this.checkList.length < this.value.length;
    },
