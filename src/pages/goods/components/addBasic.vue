<template>
    <div>
        <el-form-item class="le-basic__name">
            ...
            <el-input type="textarea" placeholder="请输入商品名称" v-model="value.name" rows="3" maxlength="40" show-word-limit>
            </el-input>
        </el-form-item>
        <el-form-item class="he-class">
            ...
            <template v-for="(item, index) in catObject.result">
                <el-tag type="info" class="le-cat__footer-tag" size="medium" closable @close="deleteTag(item, index)" :key="index" v-if="!item.children">
                    {{ item.value }}
                </el-tag>
            </template>
            <el-button type="primary" plain class="he-class__button" action="getGroup" v-popup.chooseCategory="catObject" title="选择分类" width="801" module="goods">选择分类
            </el-button>
        ...
        <el-form-item class="le-banner">
            ...
            <vuedraggable v-model="value.slideshow">
                <div class="le-label__require-item" :class="{'le-label__require-image':index===0}" v-for="(item, index) in value.slideshow" :key="index">
                        ...
                        <pictureDialog v-model="value.slideshow[index]" :limit="1">
                            ...
                            <div slot="preview" slot-scope="scope" class="select-cover__120-edit">
                                ...
                                <div class="select-cover__120-tips">
                                    <span>替换</span> | <span @click.stop="()=>{value.slideshow.splice(index, 1)}">删除</span>
                                </div>
                            </div>
                        </pictureDialog>
                    </div>
