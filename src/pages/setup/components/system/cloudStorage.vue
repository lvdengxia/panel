...
<template>
  <div class="le-main">
    ...
    >
      ...
      <template v-if="way === 0">
        <el-form-item label="" prop="" class="le-tip"
          >选择存储到本地服务器，无需配置信息</el-form-item
        >
      </template>
      <template v-if="way === 1">
        <el-form-item
        ...
        <el-form-item label="使用自定义域名" prop="is_cname" key="oss_is_cname">
          <el-switch
            :activeValue="true"
            :inactiveValue="false"
            v-model="form.oss.is_cname"
          ></el-switch>
        </el-form-item>
        <el-form-item prop="oss.domain" label="域名(Endpoint)" key="oss_domain">
          <el-input
            placeholder="请输入域名"
            v-model="form.oss.domain"
          ></el-input>
          <div class="le-prompt-text">
            示例: http://oss-xx-xxx-1.aliyuncs.com 或 http://mydomain.com
          </div>
        </el-form-item>
        <el-form-item
          label="AccessKey ID"
      ...
      <template v-if="way === 2">
        ...
        >
          <el-input
            placeholder="请输入存储空间名称"
            v-model="form.cos.bucket"
          ></el-input>
        </el-form-item>
        <el-form-item prop="cos.region" label="所属地域" key="cos_region">
          <el-input
            placeholder="请输入所属地域"
            v-model="form.cos.region"
          ></el-input>
          <div class="le-prompt-text">
            示例: ap-shanghai 或 ap-shenzhen 或 ap-xxxxxx
          </div>
        </el-form-item>
        <el-form-item label="自定义域名" prop="" key="cos_domain">
          <el-input
            v-model="form.cos.domain"
            placeholder="请输入域名"
          ></el-input>
      ...
      <template v-if="way === 3">
        ...
        >
          <el-input
            placeholder="请输入存储空间名称"
            v-model="form.qiniu.bucket"
          ></el-input>
        </el-form-item>
        <el-form-item prop="qiniu.domain" label="绑定域名" key="qinin_domain">
          <el-input
            placeholder="请输入域名"
            v-model="form.qiniu.domain"
          ></el-input>
          <div class="le-prompt-text">
            示例: http://mydomain.com 或 http://xxxxxx.bkt.clouddn.com
          </div>
        </el-form-item>
        <el-form-item
          label="AccessKey(AK)"
          prop="qiniu.access_key"
          key="qiniu_access_key"
        >
          <el-input
            v-model="form.qiniu.access_key"
            placeholder="请输入AccessKey(AK)"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="SecretKey(SK)"
          prop="qiniu.secret_key"
          key="qiniu_secret_key"
        >
          <el-input
            v-model="form.qiniu.secret_key"
            placeholder="请输入SecretKey(SK)"
          ></el-input>
        </el-form-item>
      </template>
    </el-form>
