<template>
    <div class="le-matter">
        <el-dialog class="update-dialog" title="系统是否进行二次开发？" :visible.sync="dialogVisible" width="420px" append-to-body :before-close="(done)=>{done()}">
        ...
        <div class="le-body">
            <el-scrollbar style="height: 100%">
                <div class="le-card">
                    <div class="le-time-item" v-for="(item, index) in list" :key="index" :class="list.length-1 === index ? '' : 'le-border'">
                        ...
                        <div class="le-content">
                            <div class="flex align-center le-content-header">
                                <button v-if="item.is_last_version" class="le-button" @click="dialogVisible = true">
                                    一键更新
                                </button>
                    ...
                    <div class="flex justify-center le-pagination">
                        <el-pagination background layout="prev, pager, next, jumper" @current-change="currentChange" :current-page="pagination.current" :page-count="pagination.count">
                        </el-pagination>
                    </div>
                </div>
            </el-scrollbar>
    ...
    methods: {
        ...
        toUpdatee() {
            ...
            this.$confirm('<h3 style="text-align:center;height:22px; line-height:22px;vertical-align:top; font-size:18px;font-weight: 400;color: #262626;"><i style="color:#FF8226;font-size:22px; padding-right:12px;" class="le-icon le-icon-zhushi"></i><span style="vertical-align:middle">注意</span></h3> <p style="color: #8C8C8C;font-size: 12px;">二次开发后请慎用一键更新功能！由于二次开发所导致的系统问题，官方无法处理！</p>', '', {
                ...
                .then(() => {
                    this.$heshop.cloud('post', { 'action': 'update' }).then(function(res) {
                        let token = res;
