<template>
  <div class="le-matter" v-loading="loading">
      ...
      <el-col :span="8">
        <div class="le-card he-do-info">
          <div style="height: 150px">
            ...
            <div class="he-cancel" v-if="detail.status === 102">
              订单超时未付款
            </div>
            ...
            <template v-if="detail.status === 100">
              ...
              <he-link href="setup/index">
                <el-button type="text" class="he-goSetting he-purple"
                  >去设置</el-button
                >
              </he-link>
          ...
          >
            <el-button class="he-do-btn" :disabled="afterShow(detail)"
              >确认收货</el-button
            >
          </popconfirm>
        </div>
        ...
        <div class="le-card he-step flex align-center justify-around">
          ...
          <div class="he-transition">
            <div class="he-dot-bar el-row--flex is-justify-space-around">
              <div
                class="he-dots"
                :class="detail.status > 100 ? 'he-select' : 'he-noSelect'"
              ></div>
              <div
                class="he-dots"
                :class="detail.status > 100 ? 'he-select' : 'he-noSelect'"
            ...
            <div
              class="he-row"
              :class="detail.status > 100 ? 'he-row-select' : 'he-row-noSelect'"
            ></div>
          </div>
          <template
            v-if="
          ...
          >
            ...
            <template v-if="!isAfterOver">
              ...
              <div class="he-transition">
                <div class="he-dot-bar el-row--flex is-justify-space-around">
                  <div
                    class="he-dots"
                    :class="detail.status >= 203 ? 'he-select' : 'he-noSelect'"
                  ></div>
                  <div
                    class="he-dots"
                    :class="detail.status >= 203 ? 'he-select' : 'he-noSelect'"
                ...
                <div
                  class="he-row"
                  :class="
                    detail.status >= 203 ? 'he-row-select' : 'he-row-noSelect'
                  "
                ></div>
              </div>
            ...
            <template v-if="isAfterOver">
              ...
              <template v-if="detail.status >= 203 && detail.received_time">
                <div class="he-transition">
                  <div class="he-dot-bar el-row--flex is-justify-space-around">
                    <div
                      class="he-dots he-select"
                      :class="detail.status >= 203 ? '' : 'he-noSelect'"
                    ></div>
                    <div
                      class="he-dots"
                      :class="
                        detail.status >= 203 ? 'he-select' : 'he-noSelect'
                      "
                    ></div>
                    <div
                      class="he-dots"
            ...
            <div class="he-transition">
              <div class="he-dot-bar el-row--flex is-justify-space-around">
                <div
                  class="he-dots"
                  :class="detail.status === 204 ? 'he-select' : 'he-noSelect'"
                ></div>
                <div
                  class="he-dots"
                  :class="detail.status === 204 ? 'he-select' : 'he-noSelect'"
              ...
              <div
                class="he-row"
                :class="
                  detail.status >= 204 ? 'he-row-select' : 'he-row-noSelect'
                "
              ></div>
            </div>
            <div class="he-successful-transaction">
              <div
                class="he-img__box el-row--flex is-justify-center is-align-middle"
                :class="
                  detail.status === 204 ? 'he-img__select' : 'he-img__noActive'
                "
              >
    ...
    <template v-if="detail.status >= 202 && detail.freight">
      <el-row class="le-line"></el-row>
      <div class="le-card">
        <el-row class="flex he-info">
          <el-col
            :span="6"
            :class="detail.freight.type == 1 ? 'he-form__line' : ''"
          >
            <div class="he-title">
              <span>物流信息</span>
            </div>
            <div class="he-info__content">
              ...
              <div
                ...
                v-if="detail.freight.type === 1"
              >
                <div>
                  <span class="he-logistics__title">物流公司：</span>
                  <span class="he-logistics__value">{{
                    detail.freight.logistics_company
                  }}</span>
                </div>
                <span class="he-logistics__title">快递单号：</span>
                <span class="he-logistics__value" style="margin-right: 0">{{
                  detail.freight.freight_sn
                }}</span>
                <el-button
                  class="he-button"
                  type="text"
                ...
                <input
                  ...
                  :value="detail.freight.freight_sn"
                />
              </div>
            </div>
          </el-col>
    ...
    <el-row class="he-info">
      <el-col>
        <div class="le-card">
          <el-row class="flex">
            <el-col
              class="he-info__box he-form__line"
              :span="isPay(detail.status) ? 6 : 8"
            >
              <div class="he-title">
                <span>收货信息</span>
                <he-icon
              ...
              <template slot-scope="scope">
                <div class="el-row--flex">
                  <div class="he-goods__img__box">
                    ...
                    />
                    <div
                      class="he-goods__after"
                      v-if="scope.row.after_sales"
                      @click="router(scope.row.id)"
                    >
                      {{ scope.row.after | getAfter }}
                      <he-icon
                        class="he-goods__after__icon"
                        type="le-icon-arrow-right"
                      ></he-icon>
                    </div>
                  </div>
            ...
            <div>
              <div>
                ...
                <span class="he-money"
                  >￥{{
                    Math.floor(
                      (detail.goods_amount * 1 + detail.goods_reduced * 1) * 100
                    ) / 100
                  }}</span
                >
              </div>
              <div>
                ...
                <span class="he-money"
                  >￥{{
                    Math.floor(
                      (detail.freight_amount * 1 + detail.freight_reduced * 1) *
                        100
                    ) / 100
                  }}</span
                >
              ...
              <div v-if="Number(detail.coupon_reduced)">
                ...
                <span class="he-money"
                  >-￥{{
                    Math.floor(detail.coupon_reduced * 1 * 100) / 100
                  }}</span
                >
              </div>
              ...
              <div v-if="detail.status > 200">
                <span class="he-label">实付金额:</span>
                <span class="he-money he-amount-price"
                  >￥{{ detail.pay_amount }}</span
                >
              </div>
            </div>
          </div>
        ...
        .then((res) => {
          ...
          if (res.status >= 202 && res.freight && res.freight.type == 1) {
            this.$heshop
              ...
              .then(function (res) {
                _this.logistics.list = res.list;
                _this.logistics.state = res.state;
                _this.logistics.message = res.message;
              })
              .catch(function (err) {
    ...
    afterShow: function (item) {
      ...
      for (let i = 0; i < goods.length; i++) {
        status =
          ...
          (goods[i].after.status >= 200 &&
            goods[i].after.return_number != goods[i].goods_number) ||
