  ...
  <div class="le-matter le-promote flex" v-loading="loading">
    <div class="le-menu">
      <div
        class="le-menu__item"
        @click="active = 0"
        :class="{ 'le-menu__active': active === 0 }"
      >
        微信小程序
      </div>
      <div
        class="le-menu__item"
        @click="active = 1"
        :class="{ 'le-menu__active': active === 1 }"
      >
        公众号
      </div>
    </div>
    <div
      class="le-not-config flex flex-direction align-center"
      v-if="active === 0 && notConfig"
    >
      <div class="le-title flex align-center">
        <he-icon
          type="le-icon-zhushi"
          class="le-icon-zhushi"
          size="32px"
        ></he-icon>
        <span>提示</span>
      </div>
      ...
      <div class="le-poster" :style="[height]">
              ...
              <div>
                <div class="le-price">
                  ...
                  <div>
                    ...
                    <span class="le-sell-number" v-if="(newData.sales + newData.virtual_sales) > 0">已售{{ newData.sales + newData.virtual_sales }}</span>
                  </div>
                </div>
                <div class="le-name">
        ...
        <div class="le-zoom" id="poster-coupon" v-if="promoteType === 'coupon'">
          <div class="le-poster__content">
            ...
            <div class="le-poster-coupon">
              ...
              <div class="le-content le-content-number">
                ...
                <span class="le-content-condition">{{
                  Number(newData.min_price) === 0
                    ? "无门槛使用"
                    : "满" + newData.min_price + "可用"
                }}</span>
              </div>
              <div class="le-content le-conetent-name">{{ newData.name }}</div>
              <div class="le-content le-content-date">
                可用时间:<template v-if="newData.expire_type === 1"
                  >领取当日起{{ newData.expire_day }}天内</template
            ...
            <div
              class="le-qrcode-conent flex flex-direction align-center justify-center"
            >
...
export default {
  ...
  methods: {
      ...
      return new Promise(function (resolve, reject) {
        _this.$heshop
          .search(
            "post",
            { include: "setting" },
            { keyword: "setting_collection", content_key: "store_setting" }
          )
          .then((res) => {
    ...
    copy: function () {
      ...
      copy({
        content:
          _this.active === 0 ? _this.qrcode.weapp.url : _this.qrcode.wechat.url,
        success: function () {
